...
namespace AwsDotnetCsharp
{
	...
	public partial class UserSessionData : IUnique<long>
	{
		...
		private static IMongoCollection<UserSessionData> collection
			=> _collection ?? (_collection = mongoDatabase.GetCollection<UserSessionData>("UserSessionDatas"));
			...
			UserSessionData data)
		{
			...
			if (result) { userUpdateCache.UserSessionDataTableUpdate.Upsert(data); }
			return result;
		}
			...
			IEnumerable<UserSessionData> dataList)
		{
			...
			if (result) { userUpdateCache.UserSessionDataTableUpdate.Upsert(dataList); }
			return result;
		}
		#endregion
		#region MongoDb
			...
			long id)
		{
			...
			if (result) { userUpdateCache.UserSessionDataTableUpdate.Delete(id); }
			return result;
		}
			...
			IEnumerable<long> ids)
		{
			...
			var result = deleteResult.IsAcknowledged;
			if (result) { userUpdateCache.UserSessionDataTableUpdate.Delete(ids); }
			return result;
		}
		#endregion
		#region NullObject
