...
namespace AwsDotnetCsharp
{
	...
	public partial class MissionData : IUnique<long>
	{
		...
		private static IMongoCollection<MissionData> collection
			=> _collection ?? (_collection = mongoDatabase.GetCollection<MissionData>("MissionDatas"));
			...
			MissionData data)
		{
			...
			if (result) { userUpdateCache.MissionDataTableUpdate.Upsert(data); }
			return result;
		}
			...
			IEnumerable<MissionData> dataList)
		{
			...
			if (result) { userUpdateCache.MissionDataTableUpdate.Upsert(dataList); }
			return result;
		}
		#endregion
		#region MongoDb
			...
			long id)
		{
			...
			if (result) { userUpdateCache.MissionDataTableUpdate.Delete(id); }
			return result;
		}
			...
			IEnumerable<long> ids)
		{
			...
			var result = deleteResult.IsAcknowledged;
			if (result) { userUpdateCache.MissionDataTableUpdate.Delete(ids); }
			return result;
		}
		#endregion
		#region NullObject
