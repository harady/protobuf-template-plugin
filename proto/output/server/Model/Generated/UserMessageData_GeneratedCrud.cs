...
namespace AwsDotnetCsharp
{
	...
	public partial class UserMessageData : IUnique<long>
	{
		...
		private static IMongoCollection<UserMessageData> collection
			=> _collection ?? (_collection = mongoDatabase.GetCollection<UserMessageData>("UserMessageDatas"));
			...
			UserMessageData data)
		{
			...
			if (result) { userUpdateCache.UserMessageDataTableUpdate.Upsert(data); }
			return result;
		}
			...
			IEnumerable<UserMessageData> dataList)
		{
			...
			if (result) { userUpdateCache.UserMessageDataTableUpdate.Upsert(dataList); }
			return result;
		}
		#endregion
		#region MongoDb
			...
			long id)
		{
			...
			if (result) { userUpdateCache.UserMessageDataTableUpdate.Delete(id); }
			return result;
		}
			...
			IEnumerable<long> ids)
		{
			...
			var result = deleteResult.IsAcknowledged;
			if (result) { userUpdateCache.UserMessageDataTableUpdate.Delete(ids); }
			return result;
		}
		#endregion
		#region NullObject
